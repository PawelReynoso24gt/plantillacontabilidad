<template>
  <div>
    ...
    <div class="anticipo-header">
      <div>
        <h2 class="anticipo-title">Anticipo sobre compras - Capilla</h2>
    ...
    <div class="division-container" v-if="mostrarDivisionCuatro">
      ...
      <div class="field-group">
        ...
        <select
          v-model.number="idCuentaBancaria"
          class="field-control"
        >
    ...
    <div v-if="showTabla" class="tabla-anticipos">
      ...
      <p
        v-if="!loading && anticipoRowsActivos.length === 0"
        class="tabla-empty"
      >
        No hay registros.
      </p>
...
export default {
  ...
  setup() {
    ...
    const enviarDatos = () => {
      ...
      // if (!nombre.value) { mostrarErrorCampo('nombre', 'Falta por llenar datos'); tieneErrores = true; } <-- Nombre puede ser opcional según el contexto
      ...
      const data = {
        ...
        nombre: nombre.value || null, // Si nombre está vacío, enviamos cadena vacía
    ...
    const saldarRegistroConfirm = async () => {
      ...
      // if (!modalData.nombre) { mostrarErrorModal('nombre', 'Falta por llenar datos'); tieneErrores = true; } <-- Nombre puede ser opcional según el contexto
