<template>
  ...
  <ReportPreviewHeader
    v-if="reporteData"
    :empresa="`BALANCE GENERAL ${selectedPeriodo.toUpperCase()} ${selectedPeriodo === 'Anual' ? '' : selectedYear}`"
  >
    <div>
...
export default {
  ...
  setup() {
    ...
    const periodoTexto = computed(() => {
      if (selectedPeriodo.value === 'Mensual') {
        return `BALANCE GENERAL DE ${selectedMes.value?.toUpperCase?.() || ''} ${selectedYear.value}`;
        ...
        return `BALANCE GENERAL ${trimestre[selectedMes.value] || ''} ${selectedYear.value}`;
      } else if (selectedPeriodo.value === 'Semestral') {
        const sem = selectedMes.value === 'Enero'
          ? 'BALANCE GENERAL PRIMER SEMESTRE'
          : 'BALANCE GENERAL SEGUNDO SEMESTRE';
        return `${sem} ${selectedYear.value}`;
      } else if (selectedPeriodo.value === 'Anual') {
        return `BALANCE GENERAL ANUAL (${fechaInicio.value} al ${fechaFin.value})`;
      }
      return '';
    });
  ...
  const buildPayload = () => {
    ...
    try {
      ...
      const metadata = {
        ...
        tipoReporte:
          selectedPeriodo.value === 'Anual'
            ...
            : `BALANCE GENERAL ${selectedPeriodo.value.toUpperCase()} ${selectedYear.value}`,
        especificacion: [
      ...
      const rows = [];
      // se quita detalle de aqui por motivos de resumen del reporte
      const pushRow = (descripcion, saldo_suma, suma, highlight = false) => {
        rows.push({ descripcion, saldo_suma, suma, ...(highlight ? { _variant: 'highlight' } : {}) });
      };
