...
### Listado + eliminar pattern on CRUD/registro forms
Ingreso/Egreso/Depósito/Retiro forms (e.g. `Accordion.vue`, `Breadcrumbs.vue`, `Buttons.vue`, `ButtonGroups.vue`, `FormControl.vue`, `Select.vue`, `ChecksRadios.vue`, `Range.vue`) additionally render a paginated table of existing records below the form (`registros`, `paginaActual`, `totalPaginas`, `cargandoRegistros`, `mostrarRegistros` toggle, `formatMonto`, `cargarRegistros(page)`) and a delete-confirmation modal (`mostrarConfirmEliminar`, `registroAEliminar`, `eliminando`, `errorEliminar`, `abrirConfirmEliminar`/`cerrarConfirmEliminar`/`confirmarEliminar`), styled via `src/styles/css/ListadoRegistrosA.css`. `cargarRegistros()` is called once in `onMounted` alongside the view's other setup calls. Deleting calls a `desactivarCA/:id`-style endpoint (soft delete) rather than a hard delete.
